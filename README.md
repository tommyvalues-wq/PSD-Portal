  ...
  if (!clientPromise) {
    clientPromise = (async () => {
      ...
      return new issuer.Client({
        ...
        redirect_uris: [process.env.ROBLOX_REDIRECT_URI],
        response_types: ['code'],
...
async function callback(req) {
  ...
  const checks = req.session.oauth || {};
  const tokenSet = await client.callback(process.env.ROBLOX_REDIRECT_URI, params, checks);
